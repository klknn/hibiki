...
bool isNoteOff(const MidiEvent& ev) {
  return (ev.type >= 0x80 && ev.type <= 0x8F) ||
         ((ev.type >= 0x90 && ev.type <= 0x9F) && ev.velocity == 0);
}
    ...
    while ((size_t)file.tellg() < trackEnd) {
      ...
      if (type == 0x80 || type == 0x90 || type == 0xA0 || type == 0xB0 ||
        ...
        if (type == 0x80 || type == 0x90) {
          MidiEvent ev;
          ev.beats = (double)currentTicks /
                     (double)ticksPerQuarter;  // Store in beats (quarter notes)
          ev.type = status;
