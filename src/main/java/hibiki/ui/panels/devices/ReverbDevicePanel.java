 ...
 */
public class ReverbDevicePanel extends AbstractDevicePanel {
  private static final Color ACCENT = new Color(0x7B68EE);
  ...
  public ReverbDevicePanel(int trackIndex, int pluginIndex) {
    ...
    KnobPanel[] knobs = new KnobPanel[7];
    for (int i = 0; i < 7; i++) {
      final int pi = paramIds[i];
      final int idx = i;
      knobs[i] = new KnobPanel(names[i], params[pi], makeFormatter(pi), ACCENT);
      knobs[i].addChangeListener(
          e -> {
            if (!updatingFromBackend) {
              params[pi] = knobs[idx].getValue();
  ...
  private ValueFormatter makeFormatter(int paramId) {
    return norm -> {
      if (paramId == PARAM_PRE_DELAY) return String.format("%.1fms", norm * 100.0);
      if (paramId == PARAM_HP_FREQ) {
        float hz = 20.0f * (float) Math.pow(25.0f, norm);
        return hz >= 1000 ? String.format("%.1fk", hz / 1000) : String.format("%.0f", hz);
      }
      if (paramId == PARAM_LP_FREQ) {
        float hz = 2000.0f * (float) Math.pow(10.0f, norm);
        return hz >= 1000 ? String.format("%.1fk", hz / 1000) : String.format("%.0f", hz);
      }
