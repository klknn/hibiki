...
AlsaPlayback::AlsaPlayback(int rate, int ch) : sample_rate(rate), channels(ch) {
  ...
  int err =
      snd_pcm_set_params(pcm_handle, SND_PCM_FORMAT_FLOAT_LE,
...
bool AlsaPlayback::is_ready() const { return pcm_handle != nullptr; }

void AlsaPlayback::write(const std::vector<float>& interleaved_data,
                         int num_frames) {
  if (!pcm_handle) return;
  snd_pcm_sframes_t frames =
      snd_pcm_writei(pcm_handle, interleaved_data.data(), num_frames);
  if (frames < 0) {
