...
public class PianoRoll extends JDialog {
  ...
  public PianoRoll(
      Frame owner, File midiFile, int trackIdx, int slotIdx, int clipIdx, float clipStartTime) {
      ...
      case CLIP_MIDI_DATA:
        {
          ...
          System.out.println(
              "[PianoRoll] Got ClipMidiData: track="
                  + data.getTrackIndex()
                  + " slot="
                  + data.getSlotIndex()
                  + " clip="
          ...
          if (data.getTrackIndex() == trackIdx) {
            boolean matches =
                (slotIdx >= 0 && data.getSlotIndex() == slotIdx)
  ...
  private void initUI() {
        ...
        new JPanel() {
          ...
          public Dimension getPreferredSize() {
            return new Dimension(
                Theme.getInstance().scale(60), NUM_KEYS * mouseHandler.getScaledKeyHeight());
          }
        ...
        new JPanel() {
          ...
          protected void paintComponent(Graphics g) {
            ...
            renderer.paintGrid(
                ...
                this,
                NUM_KEYS,
                mouseHandler.getScaledKeyHeight(),
                getTickWidth(),
                sequence,
                notes,
                isDraggingNote,
    ...
    JPanel timeRulerPanel =
        new JPanel() {
          ...
          protected void paintComponent(Graphics g) {
            super.paintComponent(g);
            renderer.paintTimeRuler(
                g, this, sequence, getTickWidth(), playheadPos, clipStartTime, bpm);
          }
        };
    timeRulerPanel.setBackground(Theme.getInstance().BG_DARKER);

    // Time ruler mouse handling for seeking
    MouseAdapter timeRulerMouse =
        new MouseAdapter() {
          @Override
          public void mousePressed(MouseEvent e) {
    ...
    gridScroll
        ...
        .addChangeListener(
            e -> {
              keysScroll
                  .getVerticalScrollBar()
                  .setValue(gridScroll.getVerticalScrollBar().getValue());
            });
    ...
    ZoomControlPanel zoomPanel =
        new ZoomControlPanel(
            ...
            2,
            30,
            Math.min(30, Math.max(2, keyHeight)),
    ...
    SwingUtilities.invokeLater(
        () -> {
          ...
          gridScroll
              .getVerticalScrollBar()
              .setValue(Math.max(0, centerY - gridScroll.getViewport().getHeight() / 2));
          gridScroll
              .getHorizontalScrollBar()
            ...
            e -> {
              if (isPlaying && autoScroll && sequence != null) {
                ...
                if (relativePos >= 0) {
                  float beatsPerSecond = bpm / 60.0f;
                  long playheadTick =
                      (long) (relativePos * beatsPerSecond * sequence.getResolution());
                  int playheadX = (int) (playheadTick * getTickWidth());
