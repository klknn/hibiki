...
/**
 * Handles rendering for PianoRoll panels: the note grid, velocity bars, time ruler, and piano key
 * labels.
 */
class PianoRollRenderer {
  ...
  /**
   * Paint the note grid with horizontal key lines, vertical grid lines, drag ghost, notes, and
   * playhead.
   */
  void paintGrid(
      Graphics g,
      ...
      List<PianoRoll.Note> notes,
      boolean isDraggingNote,
      PianoRoll.Note draggingNote,
      int dragOriginalPitch,
      long dragOriginalTick,
      float playheadPos,
      float clipStartTime,
      float bpm) {
    ...
    if (isDraggingNote && draggingNote != null && dragOriginalPitch >= 0) {
      ...
      g2.setStroke(
          new BasicStroke(
              1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[] {2, 2}, 0));
