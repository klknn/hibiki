...
/**
 * A dedicated automation editor dialog, similar to the Piano Roll for MIDI. Opens via right-click
 * on an automation lane → "Edit Automation..." Provides a full-size view with zooming, beat grid,
 * value rulers, and fine-grained point editing.
 */
public class AutomationEditorDialog extends JDialog {
  ...
  public AutomationEditorDialog(
      Frame owner,
      int trackIdx,
      int laneIdx,
      String paramName,
      List<AutomationEditor.AutoPoint> points,
      float bpm) {
    ...
    zoomOut.addActionListener(
        e -> {
          pixelsPerBeat = Math.max(10, pixelsPerBeat / 1.5f);
          editorPanel.updateSize();
          editorPanel.repaint();
        });
