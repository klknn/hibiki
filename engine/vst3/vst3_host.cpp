...
class VstEventList : public Steinberg::Vst::IEventList {
 ...
 public:
  ...
  Steinberg::tresult PLUGIN_API queryInterface(const Steinberg::TUID _iid,
                                               void** obj) override {
    QUERY_INTERFACE(_iid, obj, Steinberg::Vst::IEventList::iid,
...
class Vst3HostContext : public Steinberg::Vst::IHostApplication,
 ...
 public:
  ...
  Steinberg::tresult PLUGIN_API createInstance(Steinberg::TUID cid,
                                               ...
                                               void** obj) override {
    *obj = nullptr;
    if (Steinberg::FUnknownPrivate::iidEqual(cid,
    ...
    if (Steinberg::FUnknownPrivate::iidEqual(
            cid, Steinberg::Vst::IAttributeList::iid) &&
        Steinberg::FUnknownPrivate::iidEqual(
            iid, Steinberg::Vst::IAttributeList::iid)) {
  ...
  // IComponentHandler
  Steinberg::tresult PLUGIN_API
  beginEdit(Steinberg::Vst::ParamID tag) override {
    return Steinberg::kResultTrue;
  }
  Steinberg::tresult PLUGIN_API
  performEdit(Steinberg::Vst::ParamID tag,
...
Vst3Plugin::~Vst3Plugin() {
  ...
  if (impl->component && impl->controller) {
    ...
    impl->component->queryInterface(Steinberg::Vst::IConnectionPoint::iid,
                                    (void**)&cp1);
    impl->controller->queryInterface(Steinberg::Vst::IConnectionPoint::iid,
                                     (void**)&cp2);
    if (cp1 && cp2) {
      cp1->disconnect(cp2);
      cp2->disconnect(cp1);
...
bool Vst3Plugin::load(const std::string& path, int plugin_index,
  ...
  if (audioEffects.empty()) {
    LOG(WARNING) << "No audio effects found in '" << path << "'";
    return false;
  }
  ...
  impl->component->queryInterface(Steinberg::Vst::IAudioProcessor::iid,
                                  (void**)&impl->processor);
  if (!impl->processor) {
    impl->processor =
        factory.createInstance<Steinberg::Vst::IAudioProcessor>(info.ID());
  }
  ...
  if (impl->component->queryInterface(Steinberg::Vst::IEditController::iid,
                                      (void**)&impl->controller) !=
      Steinberg::kResultTrue) {
    Steinberg::TUID controllerCID;
    if (impl->component->getControllerClassId(controllerCID) ==
        Steinberg::kResultTrue) {
      impl->controller =
  ...
  if (impl->controller) {
    ...
    if (impl->hostContext->queryInterface(
            Steinberg::Vst::IComponentHandler::iid, (void**)&handler) ==
        Steinberg::kResultTrue) {
  ...
  int numInBuses = impl->component->getBusCount(Steinberg::Vst::kAudio,
                                                Steinberg::Vst::kInput);
  for (int i = 0; i < numInBuses; i++) {
    impl->component->activateBus(Steinberg::Vst::kAudio, Steinberg::Vst::kInput,
                                 i, true);
  }
  int numOutBuses = impl->component->getBusCount(Steinberg::Vst::kAudio,
                                                 Steinberg::Vst::kOutput);
  for (int i = 0; i < numOutBuses; i++) {
    impl->component->activateBus(Steinberg::Vst::kAudio,
                                 Steinberg::Vst::kOutput, i, true);
  }

  // Activate event buses (MIDI)
  int numInEventBuses = impl->component->getBusCount(Steinberg::Vst::kEvent,
                                                     Steinberg::Vst::kInput);
  for (int i = 0; i < numInEventBuses; i++) {
    impl->component->activateBus(Steinberg::Vst::kEvent, Steinberg::Vst::kInput,
                                 i, true);
  }
...
void Vst3Plugin::process(float** inputs, float** outputs, int numSamples,
  ...
  data.inputParameterChanges =
      nullptr;  // TODO: Implement if needed for real-time automation
...
bool Vst3Plugin::getParameterInfo(int index, VstParamInfo& info) const {
  ...
  if (impl->controller->getParameterInfo(index, vinfo) ==
      Steinberg::kResultTrue) {
    info.id = vinfo.id;
    char buf[128];
...
const std::string& Vst3Plugin::getName() const { return impl->name; }
const std::string& Vst3Plugin::getPath() const { return impl->path; }
int Vst3Plugin::getPluginIndex() const { return impl->pluginIndex; }
bool Vst3Plugin::isInstrument() const { return impl->isInstrument; }
...
std::vector<PluginDescription> Vst3Plugin::listPluginsIsolated(
    const std::string& path) {
  LOG(INFO) << "ISOLATED SCAN: Starting scan for " << path;
  std::vector<PluginDescription> plugins;
