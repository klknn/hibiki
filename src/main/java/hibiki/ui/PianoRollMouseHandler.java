...
/**
 * Handles mouse interactions on the PianoRoll grid panel: - Left-click to create notes (with
 * snap-to-grid) - Right-click to delete notes - Left-drag to move notes - Left-drag on note right
 * edge to resize - Middle-click / Ctrl+click to seek playhead - Alt+drag to copy notes
 */
class PianoRollMouseHandler {
  ...
  private void handleMouseReleased(MouseEvent e) {
    ...
    if (pr.draggingNote != null && pr.isDraggingNote && e.isAltDown()) {
      ...
      PianoRoll.Note copy =
          new PianoRoll.Note(
              pr.draggingNote.pitch,
              pr.draggingNote.startTick,
              pr.draggingNote.durationTicks,
              pr.draggingNote.velocity);
  ...
  private void handleMouseDragged(MouseEvent e) {
    ...
    if (pr.draggingNote != null) {
      ...
      if (e.isShiftDown()) {
        ...
        snapEndTick =
            Math.round((double) newEndTick / (pr.sequence.getResolution() / 4))
