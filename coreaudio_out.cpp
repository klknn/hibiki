...
struct CoreAudioPlayback::Impl {
  ...
  size_t getAvailableWrite() { return capacity - getAvailableRead() - 1; }
...
CoreAudioPlayback::CoreAudioPlayback(int rate, int ch)
...
bool CoreAudioPlayback::is_ready() const { return impl->ready; }

void CoreAudioPlayback::write(const std::vector<float>& interleaved_data,
                              int num_frames) {
