...
void sendAck(const char *cmd_type, bool success) {
  ...
  auto nf_off = hibiki::ipc::CreateNotification(
      builder, hibiki::ipc::Response_Acknowledge, ack_off.Union());
...
void sendParamList(int track_idx, int plugin_idx,
  ...
  for (const auto &p : params) {
    ...
    param_offsets.push_back(
        hibiki::ipc::CreateParamInfo(builder, p.id, name_off, p.defaultValue));
  }
  auto params_vec = builder.CreateVector(param_offsets);
  auto name_off = builder.CreateString(plugin_name);
  auto list_off = hibiki::ipc::CreateParamList(
      builder, track_idx, plugin_idx, name_off, is_instrument, params_vec);
  auto nf_off = hibiki::ipc::CreateNotification(
      builder, hibiki::ipc::Response_ParamList, list_off.Union());
  builder.Finish(nf_off);
  sendNotification(builder.GetBufferPointer(), builder.GetSize());
}
...
void sendLog(const std::string &msg) {
  ...
  auto nf_off = hibiki::ipc::CreateNotification(
      builder, hibiki::ipc::Response_Log, log_off.Union());
  builder.Finish(nf_off);
  sendNotification(builder.GetBufferPointer(), builder.GetSize());
}
...
void sendClipInfo(int track_idx, int slot_index, const std::string &name) {
  ...
  auto clip_off =
      hibiki::ipc::CreateClipInfo(builder, track_idx, slot_index, name_off);
  auto nf_off = hibiki::ipc::CreateNotification(
      builder, hibiki::ipc::Response_ClipInfo, clip_off.Union());
  builder.Finish(nf_off);
  sendNotification(builder.GetBufferPointer(), builder.GetSize());
}
...
void sendClearProject() {
  ...
  auto nf_off = hibiki::ipc::CreateNotification(
      builder, hibiki::ipc::Response_ClearProject, clear_off.Union());
  builder.Finish(nf_off);
  sendNotification(builder.GetBufferPointer(), builder.GetSize());
}
...
struct Clip {
  ...
  std::vector<float> audio_data; // Mono or interleaved stereo, but we'll assume
                                 // stereo for simplicity or handle both
  int num_channels = 0;
...
bool loadWav(const std::string &path, std::vector<float> &out_data,
             int &out_channels, double &out_duration) {
  ...
  while (f.read(chunkId, 4)) {
    ...
    if (std::string(chunkId, 4) == "fmt ") {
      ...
      if (bits_per_sample != 16)
        return false; // Only 16-bit supported
      if (size > 16)
...
>>>>>>> 47601e7bb99debf560fbb194795a6862d325182c
  ...
  bool load_clip(int slot, const std::string &path, bool is_loop = false) {
    ...
    if (path.size() > 4 && path.substr(path.size() - 4) == ".wav") {
      if (!loadWav(path, clip->audio_data, clip->num_channels,
                   clip->duration_sec)) {
        return false;
      }
      ...
      if (!clip->midi_events.empty()) {
        clip->duration_sec =
            clip->midi_events.back().seconds + 0.1; // Small buffer
      }
    }
    ...
    if (clip->type == Clip::AUDIO && !clip->audio_data.empty()) {
      ...
      auto wf_off =
          hibiki::ipc::CreateClipWaveform(builder, index, slot, wf_vec);
      auto nf_off = hibiki::ipc::CreateNotification(
          builder, hibiki::ipc::Response_ClipWaveform, wf_off.Union());
      builder.Finish(nf_off);
      sendNotification(builder.GetBufferPointer(), builder.GetSize());
    }
...

    {
      ...
      for (auto &pair : state.tracks) {
        ...
        context.projectTimeMusic =
            track->current_time_sec * (context.tempo / 60.0);
        ...
        if (clip->type == Clip::MIDI) {
          ...
          while (search_idx < num_midi_events) {
            ...
            if (me.seconds >= track->current_time_sec + time_per_block)
              break;
            ...
            if (me.seconds >= track->current_time_sec) {
              if (hbk::isNoteOn(me) || hbk::isNoteOff(me)) {
                MidiNoteEvent e;
                e.sampleOffset =
                    std::max(0, (int)((me.seconds - track->current_time_sec) *
                                      sample_rate));
          ...
          for (size_t i = 0; i < track->plugins.size(); ++i) {
            ...
            if (i == 0 && p->isInstrument()) {
              p->process(nullptr, outChannels, block_size, context,
                         blockEvents);
            } else {
          ...
          for (size_t i = 0; i < track->plugins.size(); ++i) {
            auto &p = track->plugins[i];
            if (p->isInstrument())
              continue; // Audio clips bypass instruments
            p->process(outChannels, outChannels, block_size, context, {});
          }
        }
...
>>>>>>> 47601e7bb99debf560fbb194795a6862d325182c
        ...
        if (track->current_time_sec >= clip->duration_sec) {
          if (clip->is_loop) {
            track->current_time_sec =
                fmod(track->current_time_sec, clip->duration_sec);
    ...
    if (++level_counter >= 4) {
      ...
      std::vector<flatbuffers::Offset<hibiki::ipc::TrackLevel>> level_offsets;
      {
        ...
        for (auto &pair : state.track_levels) {
          level_offsets.push_back(hibiki::ipc::CreateTrackLevel(
              builder, pair.first, pair.second.first, pair.second.second));
        }
      }
      auto levels_vec = builder.CreateVector(level_offsets);
      auto levels_off = hibiki::ipc::CreateTrackLevels(builder, levels_vec);
      auto nf_off = hibiki::ipc::CreateNotification(
          builder, hibiki::ipc::Response_TrackLevels, levels_off.Union());
      builder.Finish(nf_off);
      // We need a thread-safe way to send notifications if main thread is also
      // sending For now, assume playback_thread can write to cout too, but we
      // should be careful with mutex Actually main.cpp sendNotification doesn't
      // have a mutex. Let's add one.
      sendNotification(builder.GetBufferPointer(), builder.GetSize());
    }
...
void save_project(GlobalState &state, const std::string &path) {
  ...
  for (auto &pair : state.tracks) {
    ...
    for (auto &plugin : track->plugins) {
      ...
      for (int i = 0; i < plugin->getParameterCount(); i++) {
        ...
        if (plugin->getParameterInfo(i, info)) {
          params.push_back(hibiki::project::CreateParameter(
              builder, info.id, (float)plugin->getParameterValue(info.id)));
        }
      }
      auto params_vec = builder.CreateVector(params);
      auto path_off = builder.CreateString(plugin->getPath());
      auto p = hibiki::project::CreatePlugin(
          builder, path_off, plugin->getPluginIndex(), params_vec);
    ...
    for (auto const &[slot, clip] : track->clips) {
      ...
      auto type = (clip->type == Clip::AUDIO) ? hibiki::project::ClipType_AUDIO
                                              : hibiki::project::ClipType_MIDI;
      clip_offsets.push_back(hibiki::project::CreateClip(
          builder, slot, path_off, clip->is_loop, type));
    }

    auto plugins_vec = builder.CreateVector(plugin_offsets);
    auto clips_vec = builder.CreateVector(clip_offsets);
    track_offsets.push_back(hibiki::project::CreateTrack(
        builder, track->index, plugins_vec, clips_vec));
  }

  auto tracks_vec = builder.CreateVector(track_offsets);
  auto project =
      hibiki::project::CreateProject(builder, (float)state.bpm, tracks_vec);
    ...
    for (auto const &track_fb : *project->tracks()) {
      ...
      if (track_fb->plugins()) {
        for (auto const &plugin_fb : *track_fb->plugins()) {
          int idx =
              track->load_plugin(plugin_fb->path()->str(), plugin_fb->index());
          if (idx != -1) {
            ...
            sendParamList(track->index, idx, plugin->getName(),
...
>>>>>>> 47601e7bb99debf560fbb194795a6862d325182c
  ...
  while (true) {
    ...
    if (command_type == hibiki::ipc::Command_LoadPlugin) {
      ...
      auto cmd = request->command_as_SetClipLoop();
      state.get_or_create_track(cmd->track_index())
          ->set_clip_loop(cmd->slot_index(), cmd->is_loop());
      sendAck("SET_CLIP_LOOP", true);
    } else if (command_type == hibiki::ipc::Command_Play) {
      sendAck("PLAY", true);
    } else if (command_type == hibiki::ipc::Command_Stop) {
