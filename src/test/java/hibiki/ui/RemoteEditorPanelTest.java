...
/**
 * Unit tests for RemoteEditorPanel. Tests proto construction and frame decoding without needing a
 * running backend.
 */
public class RemoteEditorPanelTest {
        ...
        Request.newBuilder()
            .setPlugin(
                PluginCmd.newBuilder()
                    ...
                    .setTarget(EntityRef.newBuilder().setTrackIndex(2).setPluginIndex(1)))
            .build();

    assertTrue(req.hasPlugin());
    assertEquals(PluginCmd.Action.ACTION_GET_EDITOR_FRAME, req.getPlugin().getAction());
  ...
  public void testEditorInputRequestConstruction() {
    ...
    assertTrue(req.hasPlugin());
    assertEquals(PluginCmd.Action.ACTION_SEND_EDITOR_INPUT, req.getPlugin().getAction());
  ...
  public void testStopGuiRequestConstruction() {
    ...
    assertTrue(req.hasPlugin());
    assertEquals(PluginCmd.Action.ACTION_STOP_GUI, req.getPlugin().getAction());
  }
  ...
  public void testEditorFrameDataParsing() {
    ...
    byte[] rgba =
        new byte[] {
          ...
          (byte) 255,
          0,
          (byte) 255 // Yellow pixel
        };

    EditorFrameData frame =
        EditorFrameData.newBuilder()
            .setTrackIndex(0)
            .setPluginIndex(0)
            .setWidth(2)
            .setHeight(2)
    ...
    Notification notif = Notification.newBuilder().setEditorFrameData(frame).build();
