...
class BuiltinDr8CowbellTest : public ::testing::Test {
 protected:
  BuiltinDr8Cowbell plugin;

  void SetUp() override { plugin.load(BuiltinDr8Cowbell::kPath, 0, 44100.0); }
};

TEST_F(BuiltinDr8CowbellTest, MetadataAndParams) {
  EXPECT_EQ(plugin.getName(), "DR8 Cowbell");
  EXPECT_EQ(plugin.getPath(), "builtin://dr8_cowbell");
...
TEST_F(BuiltinDr8CowbellTest, PlayAndDecay) {
  ...
  // Trigger Cowbell note
  plugin.setParameterValue(BuiltinDr8Cowbell::PARAM_DECAY, 0.0);  // fast decay
  MidiNoteEvent note_on{};
  note_on.isNoteOn = true;
  note_on.pitch = 56;  // Cowbell G#2
  note_on.velocity = 0.9f;
  events.push_back(note_on);

  plugin.process(nullptr, outputs, 500, context, events);

  // Output should have non-zero cowbell sound
  ...
  EXPECT_TRUE(has_sound) << "Cowbell should produce sound when triggered";
  ...
  for (int i = 0; i < 500; ++i) {
    EXPECT_NEAR(output_l[i], 0.0f, 0.0001f)
        << "Cowbell should decay to silence after playing out, sample " << i;
  }
}
